...
#	Description: ServerStatus中文版 服务端 + 客户端一键安装管理脚本 
#	Version: 1.0.1
#	Author: 7colorblog
#	Blog: https://www.7colorblog.com
#=================================================
...
Download_Server_Status_server(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/lizhongnian/ServerStatus/archive/master.zip"
...
Download_Server_Status_client(){
	...
	[[ ! -e ${file} ]] && mkdir "${file}"
	cd "${file}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/lizhongnian/ServerStatus/master/clients/client-linux.py"
...
Service_Server_Status_server(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/lizhongnian/7colorblog/master/other/server_status_server_centos" -O /etc/init.d/status-server; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/lizhongnian/7colorblog/master/other/server_status_server_debian" -O /etc/init.d/status-server; then
			echo -e "${Error} ServerStatus 服务端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-server
...
Service_Server_Status_client(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/lizhongnian/7colorblog/master/other/server_status_client_centos" -O /etc/init.d/status-client; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/lizhongnian/7colorblog/master/other/server_status_client_debian" -O /etc/init.d/status-client; then
			echo -e "${Error} ServerStatus 客户端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-client
...
默认为本机IP为域名，例如输入: status.7colorblog.com，如果要使用本机IP，请留空直接回车"
...
Set_password(){
	...
	stty erase '^H' && read -p "(默认: 98k.li):" password_s
	[[ -z "$password_s" ]] && password_s="98k.li"
	echo && echo "	================================================"
...
Install_caddy(){
	...
	if [[ "${caddy_yn}" == [Yy] ]]; then
		if [[ ! -e "/usr/local/caddy/caddy" ]]; then
			wget -N --no-check-certificate https://raw.githubusercontent.com/lizhongnian/7colorblog/master/caddy_install.sh
...
menu_client(){
echo && echo -e "  ServerStatus中文版 一键安装管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
